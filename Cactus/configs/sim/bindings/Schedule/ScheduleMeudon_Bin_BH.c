/*@@
   @file       ScheduleMeudon_Bin_BH.c
   @author     Automatically generated by CreateScheduleBindings.pl
   @desc
               Creates the schedule and parameter recovery bindings 
               for thorn Meudon_Bin_BH
   @enddesc
@@*/


#include "Meudon_Bin_BH/cctk.h"
#include "Meudon_Bin_BH/CParameters.h"
#include "cctki_ScheduleBindings.h"
#include "Meudon_Bin_BH/cctk_ScheduleFunctions.h"

/* Prototypes for Fortran schedule bindings functions to be registered */
/* Note that this is a cheat, we just need a function pointer. */


void CCTKi_BindingsSchedule_Meudon_Bin_BH(void);
void CCTKi_BindingsSchedule_Meudon_Bin_BH(void)
{
  DECLARE_CCTK_PARAMETERS
if (CCTK_EQUALS (initial_data,    "ID_Bin_BH") ||
    CCTK_EQUALS (initial_lapse,   "ID_Bin_BH") ||
    CCTK_EQUALS (initial_shift,   "ID_Bin_BH") ||
    CCTK_EQUALS (initial_dtlapse, "ID_Bin_BH") ||
    CCTK_EQUALS (initial_dtshift, "ID_Bin_BH"))
{
  {
    int cctkschedulei_tlevelarray[] = {0};
    CCTKi_ScheduleFunction((void *)ID_Bin_BH_check_parameters,
                           "ID_Bin_BH_check_parameters",
                           "Meudon_Bin_BH",
                           "ID_Bin_BH",
                           "Check parameters",
                           "CCTK_PARAMCHECK",
                           "C",
                           0,  /* Number of STORAGE  groups   */
                           0,  /* Number of COMM     groups   */
                           0,  /* Number of TRIGGERS groups   */
                           0, /* Number of SYNC     groups    */
                           0, /* Number of WRITES clauses     */
                           0, /* Number of READS clauses      */
                           0, /* Number of Options            */
                           0, /* Number of BEFORE   routines  */
                           0, /* Number of AFTER    routines  */
                           0, /* Number of WHILE    variables */
                           0, /* Number of IF       variables */
                           cctkschedulei_tlevelarray  /* Array of timelevel data for storage groups */,
                           "");
  }
  {
    int cctkschedulei_tlevelarray[] = {0};
    CCTKi_ScheduleFunction((void *)ID_Bin_BH_initialise,
                           "ID_Bin_BH_initialise",
                           "Meudon_Bin_BH",
                           "ID_Bin_BH",
                           "Set up binary black hole initial data",
                           "ADMBase_InitialData",
                           "C",
                           0,  /* Number of STORAGE  groups   */
                           0,  /* Number of COMM     groups   */
                           0,  /* Number of TRIGGERS groups   */
                           0, /* Number of SYNC     groups    */
                           0, /* Number of WRITES clauses     */
                           0, /* Number of READS clauses      */
                           0, /* Number of Options            */
                           0, /* Number of BEFORE   routines  */
                           0, /* Number of AFTER    routines  */
                           0, /* Number of WHILE    variables */
                           0, /* Number of IF       variables */
                           cctkschedulei_tlevelarray  /* Array of timelevel data for storage groups */,
                           "");
  }
}
}

/*@@
  @routine    CCTKi_BindingsParameterRecovery_Meudon_Bin_BH
  @author     Automatically generated by CreateScheduleBindings.pl
  @desc
              Creates the parameter recovery bindings for thorn Meudon_Bin_BH
  @enddesc
@@*/

int CCTKi_BindingsParameterRecovery_Meudon_Bin_BH(void);
int CCTKi_BindingsParameterRecovery_Meudon_Bin_BH(void)
{
  /* this thorn doesn't define any parameter recovery routines */
  return (0);
}

