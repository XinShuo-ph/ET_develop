/*@@
   @file       ScheduleMeudon_Bin_NS.c
   @author     Automatically generated by CreateScheduleBindings.pl
   @desc
               Creates the schedule and parameter recovery bindings 
               for thorn Meudon_Bin_NS
   @enddesc
@@*/


#include "Meudon_Bin_NS/cctk.h"
#include "Meudon_Bin_NS/CParameters.h"
#include "cctki_ScheduleBindings.h"
#include "Meudon_Bin_NS/cctk_ScheduleFunctions.h"

/* Prototypes for Fortran schedule bindings functions to be registered */
/* Note that this is a cheat, we just need a function pointer. */


void CCTKi_BindingsSchedule_Meudon_Bin_NS(void);
void CCTKi_BindingsSchedule_Meudon_Bin_NS(void)
{
  DECLARE_CCTK_PARAMETERS
if (CCTK_EQUALS (initial_data,    "Meudon_Bin_NS") ||
    CCTK_EQUALS (initial_lapse,   "Meudon_Bin_NS") ||
    CCTK_EQUALS (initial_shift,   "Meudon_Bin_NS") ||
    CCTK_EQUALS (initial_dtlapse, "Meudon_Bin_NS") ||
    CCTK_EQUALS (initial_dtshift, "Meudon_Bin_NS") ||
    CCTK_EQUALS (initial_hydro,   "Meudon_Bin_NS"))
{
  {
    int cctkschedulei_tlevelarray[] = {0};
    CCTKi_ScheduleFunction((void *)Meudon_Bin_NS_initialise,
                           "Meudon_Bin_NS_initialise",
                           "Meudon_Bin_NS",
                           "Meudon_Bin_NS",
                           "Set up binary neutron star initial data",
                           "HydroBase_Initial",
                           "C",
                           0,  /* Number of STORAGE  groups   */
                           0,  /* Number of COMM     groups   */
                           0,  /* Number of TRIGGERS groups   */
                           0, /* Number of SYNC     groups    */
                           0, /* Number of WRITES clauses     */
                           0, /* Number of READS clauses      */
                           0, /* Number of Options            */
                           0, /* Number of BEFORE   routines  */
                           0, /* Number of AFTER    routines  */
                           0, /* Number of WHILE    variables */
                           0, /* Number of IF       variables */
                           cctkschedulei_tlevelarray  /* Array of timelevel data for storage groups */,
                           "");
  }
}
}

/*@@
  @routine    CCTKi_BindingsParameterRecovery_Meudon_Bin_NS
  @author     Automatically generated by CreateScheduleBindings.pl
  @desc
              Creates the parameter recovery bindings for thorn Meudon_Bin_NS
  @enddesc
@@*/

int CCTKi_BindingsParameterRecovery_Meudon_Bin_NS(void);
int CCTKi_BindingsParameterRecovery_Meudon_Bin_NS(void)
{
  /* this thorn doesn't define any parameter recovery routines */
  return (0);
}

