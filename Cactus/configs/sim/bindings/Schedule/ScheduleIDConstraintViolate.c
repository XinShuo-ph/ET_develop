/*@@
   @file       ScheduleIDConstraintViolate.c
   @author     Automatically generated by CreateScheduleBindings.pl
   @desc
               Creates the schedule and parameter recovery bindings 
               for thorn IDConstraintViolate
   @enddesc
@@*/


#include "IDConstraintViolate/cctk.h"
#include "IDConstraintViolate/CParameters.h"
#include "cctki_ScheduleBindings.h"
#include "IDConstraintViolate/cctk_ScheduleFunctions.h"

/* Prototypes for Fortran schedule bindings functions to be registered */
/* Note that this is a cheat, we just need a function pointer. */
extern int CCTK_FNAME(IDConstraintViolate_Initial)(void);

void CCTKi_BindingsSchedule_IDConstraintViolate(void);
void CCTKi_BindingsSchedule_IDConstraintViolate(void)
{
  DECLARE_CCTK_PARAMETERS
  {
    int cctkschedulei_tlevelarray[] = {0};
    CCTKi_ScheduleFunction((void *)IDConstraintViolate_ParamChecker,
                           "IDConstraintViolate_ParamChecker",
                           "IDConstraintViolate",
                           "idconstraintviolate",
                           "Check parameters",
                           "CCTK_PARAMCHECK",
                           "C",
                           0,  /* Number of STORAGE  groups   */
                           0,  /* Number of COMM     groups   */
                           0,  /* Number of TRIGGERS groups   */
                           0, /* Number of SYNC     groups    */
                           0, /* Number of WRITES clauses     */
                           0, /* Number of READS clauses      */
                           0, /* Number of Options            */
                           0, /* Number of BEFORE   routines  */
                           0, /* Number of AFTER    routines  */
                           0, /* Number of WHILE    variables */
                           0, /* Number of IF       variables */
                           cctkschedulei_tlevelarray  /* Array of timelevel data for storage groups */,
                           "");
  }
  {
    int cctkschedulei_tlevelarray[] = {0};
    CCTKi_ScheduleFunction((void *)CCTK_FNAME(IDConstraintViolate_Initial),
                           "IDConstraintViolate_Initial",
                           "IDConstraintViolate",
                           "idconstraintviolate",
                           "Set up constraint violating initial data",
                           "ADMBase_InitialData",
                           "Fortran",
                           0,  /* Number of STORAGE  groups   */
                           0,  /* Number of COMM     groups   */
                           0,  /* Number of TRIGGERS groups   */
                           0, /* Number of SYNC     groups    */
                           0, /* Number of WRITES clauses     */
                           0, /* Number of READS clauses      */
                           0, /* Number of Options            */
                           0, /* Number of BEFORE   routines  */
                           0, /* Number of AFTER    routines  */
                           0, /* Number of WHILE    variables */
                           0, /* Number of IF       variables */
                           cctkschedulei_tlevelarray  /* Array of timelevel data for storage groups */,
                           "");
  }
}

/*@@
  @routine    CCTKi_BindingsParameterRecovery_IDConstraintViolate
  @author     Automatically generated by CreateScheduleBindings.pl
  @desc
              Creates the parameter recovery bindings for thorn IDConstraintViolate
  @enddesc
@@*/

int CCTKi_BindingsParameterRecovery_IDConstraintViolate(void);
int CCTKi_BindingsParameterRecovery_IDConstraintViolate(void)
{
  /* this thorn doesn't define any parameter recovery routines */
  return (0);
}

