/*@@
   ...
   @desc
            Creates the CCTK variables for thorn NoMPI
   @enddesc
 @@*/
...
static int CCTKi_BindingsFortranWrapperNoMPI(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperNoMPI(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(NOMPI_C2F_PROTO);
  DECLARE_NOMPI_C2F
  INITIALISE_NOMPI_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (NOMPI_C2F_PROTO)) fpointer;
  function (PASS_NOMPI_C2F (GH));
...
int CactusBindingsVariables_NoMPI_Initialise(void)
{
  ...
  CCTKi_RegisterFortranWrapper("NoMPI", CCTKi_BindingsFortranWrapperNoMPI);
