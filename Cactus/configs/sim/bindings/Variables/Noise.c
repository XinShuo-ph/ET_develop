/*@@
   ...
   @desc
            Creates the CCTK variables for thorn Noise
   @enddesc
 @@*/
...
#include "Noise/cctk_Arguments.h"
...
static int CCTKi_BindingsFortranWrapperNoise(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperNoise(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(NOISE_C2F_PROTO);
  DECLARE_NOISE_C2F
  INITIALISE_NOISE_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (NOISE_C2F_PROTO)) fpointer;
  function (PASS_NOISE_C2F (GH));
...
int CactusBindingsVariables_Noise_Initialise(void)
{
  ...
  CCTKi_RegisterFortranWrapper("Noise", CCTKi_BindingsFortranWrapperNoise);
