/*@@
   ...
   @desc
            Creates the CCTK variables for thorn HTTPD
   @enddesc
 @@*/
...
static int CCTKi_BindingsFortranWrapperHTTPD(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperHTTPD(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(HTTPD_C2F_PROTO);
  DECLARE_HTTPD_C2F
  INITIALISE_HTTPD_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (HTTPD_C2F_PROTO)) fpointer;
  function (PASS_HTTPD_C2F (GH));
...
int CactusBindingsVariables_HTTPD_Initialise(void)
{
  ...
  CCTKi_RegisterFortranWrapper("HTTPD", CCTKi_BindingsFortranWrapperHTTPD);
