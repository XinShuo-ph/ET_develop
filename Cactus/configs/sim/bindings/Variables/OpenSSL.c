/*@@
   ...
   @desc
            Creates the CCTK variables for thorn OpenSSL
   @enddesc
 @@*/
...
static int CCTKi_BindingsFortranWrapperOpenSSL(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperOpenSSL(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(OPENSSL_C2F_PROTO);
  DECLARE_OPENSSL_C2F
  INITIALISE_OPENSSL_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (OPENSSL_C2F_PROTO)) fpointer;
  function (PASS_OPENSSL_C2F (GH));
...
int CactusBindingsVariables_OpenSSL_Initialise(void)
{
  ...
  CCTKi_RegisterFortranWrapper("OpenSSL", CCTKi_BindingsFortranWrapperOpenSSL);
