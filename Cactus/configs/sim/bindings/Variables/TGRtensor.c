/*@@
   ...
   @desc
            Creates the CCTK variables for thorn TGRtensor
   @enddesc
 @@*/
...
static int CCTKi_BindingsFortranWrapperTGRtensor(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperTGRtensor(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(TGRTENSOR_C2F_PROTO);
  DECLARE_TGRTENSOR_C2F
  INITIALISE_TGRTENSOR_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (TGRTENSOR_C2F_PROTO)) fpointer;
  function (PASS_TGRTENSOR_C2F (GH));
...
int CactusBindingsVariables_TGRtensor_Initialise(void)
{
  ...
  CCTKi_RegisterFortranWrapper("TGRtensor", CCTKi_BindingsFortranWrapperTGRtensor);
