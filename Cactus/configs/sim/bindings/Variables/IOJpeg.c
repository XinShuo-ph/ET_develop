/*@@
   ...
   @desc
            Creates the CCTK variables for thorn IOJpeg
   @enddesc
 @@*/
...
static int CCTKi_BindingsFortranWrapperIOJpeg(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperIOJpeg(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(IOJPEG_C2F_PROTO);
  DECLARE_IOJPEG_C2F
  INITIALISE_IOJPEG_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (IOJPEG_C2F_PROTO)) fpointer;
  function (PASS_IOJPEG_C2F (GH));
...
int CactusBindingsVariables_IOJpeg_Initialise(void)
{
  ...
  CCTKi_RegisterFortranWrapper("IOJpeg", CCTKi_BindingsFortranWrapperIOJpeg);
