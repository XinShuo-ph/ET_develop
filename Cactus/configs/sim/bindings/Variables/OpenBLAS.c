/*@@
   ...
   @desc
            Creates the CCTK variables for thorn OpenBLAS
   @enddesc
 @@*/
...
static int CCTKi_BindingsFortranWrapperOpenBLAS(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperOpenBLAS(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(OPENBLAS_C2F_PROTO);
  DECLARE_OPENBLAS_C2F
  INITIALISE_OPENBLAS_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (OPENBLAS_C2F_PROTO)) fpointer;
  function (PASS_OPENBLAS_C2F (GH));
...
int CactusBindingsVariables_OpenBLAS_Initialise(void)
{
  ...
  CCTKi_RegisterFortranWrapper("OpenBLAS", CCTKi_BindingsFortranWrapperOpenBLAS);
